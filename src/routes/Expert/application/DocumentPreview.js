        ...
        return (
            <div class='document-view' key={document._id}>
                            ...
                            <Link to={'/expert/documents/'+document._id}>
                                {
                                   ...
                                   <div><p>{document.name}</p> <div class='pdf-view'><PdfViewer file={'http://localhost:1169/expert/documents/'+document._id+'/preview'} /></div> </div>:
                                    <img src={'http://localhost:1169/expert/documents/'+document._id+'/preview'} />
                                }
                            </Link> :
                            <Dropzone onDrop={files => uploadFiles(files, document._id)}/>
                        }
                <div class='details-mask row row-align'><span class='col-lg-6' onClick={()=>{details.style.display='block'}}>Show</span><span class='col-lg-6' onClick={()=>{details.style.display='none'}}>Hide</span></div>
                ...
                <div>
                    ...
                    <div class='row' style={{paddingLeft:'13%'}}><button type='button' onClick = {() => changeDocumentCategory(category.value, document._id )} class="btn btn-primary col-lg-5"> Move </button>
                    <button type='button' style={{marginLeft:'3%'}} onClick = {() => deleteDocument(document._id)} class="btn btn-primary col-lg-5"> Delete </button></div>
                    </div>
                </div>
            </div>
        );
